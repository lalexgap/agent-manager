...
- am role list · am role show <name>   named behavior presets. Before spawning, check whether a role matches the task (e.g. a "shepherd" role for shepherding a PR) and pass it via --role so the agent gets the role's instructions — don't paraphrase a role's job into -m yourself.
